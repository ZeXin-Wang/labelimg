...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        # wzx：界面右边BOX列表添加对鼠标ENTER的事件监听
        self.labelList.setMouseTracking(True)
        self.labelList.itemEntered.connect(self.labelEnterChanged)
        ...
        # wzx：修改editLabel功能快捷键，由Ctrl+E改为E
        edit = action(getStr('editLabel'), self.editLabel,
                      'E', 'edit', getStr('editLabelDetail'),
                      enabled=False)
        # wzx：添加X快捷键作为删除BOX，按起来方便
        delete1 = action(getStr('delBox'), self.deleteSelectedShape,
                'X', 'delete', getStr('delBoxDetail'), enabled=True)
        # wzx：添加C快捷键作为BOX复制功能(参考copyPrevBounding)
        copyCurrBounding = action(getStr('copyPrevBounding'), self.copyCurrentBoundingBoxes,
        ...
        self.actions = struct(save=save, save_format=save_format, saveAs=saveAs, open=open, close=close, resetAll = resetAll, deleteImg = deleteImg,
                              lineColor=color1, create=create, delete=delete, delete1=delete1, edit=edit, copy=copy,
                              ...
                              beginner=(), advanced=(),
                              editMenu=(edit, copy, delete, delete1,
                                        None, color1, self.drawSquaresOption),
                              beginnerContext=(create, edit, copy, delete, delete1),
                              advancedContext=(createMode, editMode, edit, copy,
                                               delete, delete1, shapeLineColor, shapeFillColor),
                              onLoadActive=(
        ...
        addActions(self.menus.file,
                   (open, opendir, copyPrevBounding, copyCurrBounding, changeSavedir, openAnnotation, self.menus.recentFiles, save, save_format, saveAs, close, resetAll, deleteImg, quit))
    ...
    # wzx：当鼠标在界面右边的BOX列表中划动时，主界面中的对应的BOX会有一定的高亮，以便知道当前所划到的BOX
    def labelEnterChanged(self, item):
        if not (item and self.canvas.editing()):
            return
        shape = self.itemsToShapes[item]
        if not self.canvas.isVisible(shape):
            return
        self.canvas.hVertex, self.canvas.hShape = None, shape
        self.canvas.update()
    ...
    def editLabel(self):
        ...
        if not item:
            return
        # wzx：保留上一次选择的label，下一次修改时可以直接选择
        if self.prevLabelEditText:
            text = self.labelDialog.popUp(self.prevLabelEditText)
        else:
            text = self.labelDialog.popUp(item.text())
        if text is not None:
            self.prevLabelEditText = text
            item.setText(text)
    ...
    # wzx：复制当前图片的BOX到下一个图片
    def copyCurrentBoundingBoxes(self):
        # 没有打开图片序列，或者当前是最后一张图片，都直接return
        if not self.filePath:
            return
        currIndex = self.mImgList.index(self.filePath)
        if currIndex >= len(self.mImgList):
            return
        # 尝试获取当前选中的目标框
        selected_shape = self.canvas.selectedShape.copy()
        self.openNextImg()
        if selected_shape is None:
            # 没有选择任何目标框，直接copy本张图片所有框到下一张
            self.copyPreviousBoundingBoxes()
        else:
            # 否则，只复制当前选中的框到下一张图片
            self.addLabel(selected_shape)
            self.canvas.shapes.append(selected_shape)
            self.canvas.selectedShape = selected_shape
            self.shapeSelectionChanged(True)
            self.canvas.update()
            self.saveFile()
